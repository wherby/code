...
def resolve():
    ...
    while st:
        #print(st)
        cost,a,idx = heapq.heappop(st)
        if dp[idx]> cost:
            dp[idx] = cost
        if cost > dp[idx-1] +2500: continue
        ...
        if bidx>0:
            if (lls[b][bidx-1],idx+1) not in visited or visited[(lls[b][bidx-1],idx+1)] > cost + abs(lls[b][bidx-1]-a)    :
                heapq.heappush(st,(cost + abs(lls[b][bidx-1]-a),lls[b][bidx-1],idx+1))
        ...
        if bidx < len(lls[b]):
            if (lls[b][bidx],idx +1) not in visited or visited[(lls[b][bidx],idx +1)] >cost +abs(lls[b][bidx]-a):
                heapq.heappush(st,(cost +abs(lls[b][bidx]-a),lls[b][bidx],idx +1))
