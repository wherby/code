...
State rot(State s, int i){
    i = ((i%N)+N)%N;
    if(!i) return s;
    return (s << i) | (s >> (N-i));
}

vector<State> precalc(){
    vector<State> ret;
    /// SOLVE HERE
    bitset<S> span;
    span[0] = 1;
    for(;;){
        ...
        for(uint32_t m = 0; m < S; ++m){
            ...
            bool me = true;
            for(int i=0; i<8; ++i){
                if(!span[(State(m) ^ rot(State(m), i)).to_ulong()]){
                    me=false;
                    break;
                }
            }
            if(me){
                bad = false;
                cerr << "add: " << State(m) << "\n";
                ret.push_back(State(m));
                for(int i=0; i<S; ++i){
                    if(span[i]) span[i^m] = 1;
                }
                break;
            }
        }
...
vector<State> pre = precalc();

void solve(){
    for(int i=S-1; i>0; --i){
        State x = pre[__builtin_ctz(i)];
        cout << x << endl;
        int ans;
        cin >> ans;
        if(ans == 0) return;
        if(ans == -1) return;
        // yes, we don't actually care about ans :)
    }
    exit(1);
}
