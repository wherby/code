...
void solve(){
  li n,p; cin>>n>>p;
  vector<tuple<li,li>> V;
  V.emplace_back(0,0);
  for(li i = 0; i < (li)(n); ++i) {
    vi X(p); for(li j = 0; j < (li)(p); ++j) cin>>X[j];
    sort(begin(X), end(X));
    li w0 = 1ll<<60;
    li w1 = 1ll<<60;
    for(auto [q,cost] : V) {
      smin(w0,cost + abs(X.back()-q) + abs(X.back()-X[0]));
      smin(w1,cost + abs(X[0]-q) + abs(X.back()-X[0]));
    }
    vector<tuple<li,li>> W = {make_tuple(X[0],w0),make_tuple(X.back(),w1)};
    V = W;
  }
  li tot = 1ll<<60;
  for(auto [q,cost] : V) {
