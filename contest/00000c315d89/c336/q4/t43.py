...
class segment_tree:
    ...
    def bound(self,val,root,l=None,r=None):
        if l ==None:
            l,r = 0,self.n-1
        if l==r:return l 
        mid = (l+r)>>1
        self.__pushDownSetValue(root,l,r)
        lc = self.__bound(2*root+1,l,mid)
        print(lc,l,r,val,root)
        if lc >= val:
            return self.bound(val,2*root+1,l,mid)
        else:
            return self.bound(val-lc,2*root+2,mid+1,r)
    
    def __bound(self,root,l,r):
        return r-l +1 -self.tree[root]
