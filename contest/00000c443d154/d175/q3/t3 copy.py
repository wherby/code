...
class Solution:
    def longestSubsequence(self, nums: List[int]) -> int:
        ...
        stls = [SegmentTree( max,0,[0]*len(ns)) for _ in range(32)]
        ret = 0
        for a in nums:
            ai = index[a]
            for bit in range(32):
                if (a>>bit)&1:
                    curv =stls[bit].get_value(ai)
                    cur = stls[bit].prod(0, ai) +1
                    if cur > curv:
                        stls[bit].set_value(ai, cur)
        for bit in range(32):
            ret = max(ret, stls[bit].prod(0, len(ns)))
        return ret
