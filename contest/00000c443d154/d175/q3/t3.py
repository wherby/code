...
class Solution:
    def longestSubsequence(self, nums: List[int]) -> int:
        ...
        stls = [segment_tree( [0]*len(ns), merge = max, basev = 0, basef=lambda x:x) for _ in range(32)]
        ret = 0
        for a in nums:
            ai = index[a]
            for bit in range(32):
                if (a>>bit)&1:
                    curv =stls[bit].array[ai]
                    cur = stls[bit].query(0, ai-1) +1
                    if cur > curv:
                        stls[bit].update(ai, cur)
        for bit in range(32):
            ret = max(ret, stls[bit].query(0, len(ns)-1))
        return ret
