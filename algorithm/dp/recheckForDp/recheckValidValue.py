...
# Recheck is need for valid value
import os
...
def resolve():
    ...
    for i in range(P):
        ...
        localDic[(x,y)] =(i,c)
    mx= -10**10
    dp =[[[[-10**10]*(N+1) for _ in range(N+1)] for _ in range(M+1)] for _ in range(1<<P)]
    #dp= defaultdict(lambda:-10*10)
    dp[0][M][Ar][Ac]  =0
    st = deque([(Ar,Ac,M,0,0)])
    while st:
        #print(len(st))
        #print(st)
        ar,ac,m,val,status = st.popleft()
        if dp[status][m][ar][ac] > val:continue  ## recheck valid value is needed
        #print(ar,ac,m,val,status,len(st))
        if (ar,ac) in localDic:
            i,cost = localDic[(ar,ac)]
            if (1<<i)&status ==0 and dp[status|(1<<i)][m][ar][ac] < val +cost:
                dp[status|(1<<i)][m][ar][ac]= val +cost
        ...
        if ar >1:
            ...
            if val1 > dp[status][m1][ar1][ac1] :
                dp[status][m1][ar1][ac1] = val1
        ...
        if ar<N:
            ...
            if val1 > dp[status][m1][ar1][ac1] :
                dp[status][m1][ar1][ac1] = val1
                st.append((ar1,ac1,m1,val1,status))
    return str(mx) if mx !=-10**10 else "IMPOSSIBLE"
