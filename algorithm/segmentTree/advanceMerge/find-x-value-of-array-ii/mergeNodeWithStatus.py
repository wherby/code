# https://leetcode.cn/contest/weekly-contest-446/ranking/?region=local_v2
# py-is-best-lang
...
class Solution:
    def resultArray(
        ...
        global K
        K = k
        tree = SegTree(op, T(), [T(x) for x in nums])
        res = []
        for i, v, s, x in queries:
            tree.set(i, T(v))
            res.append(tree.prod(s, len(nums)).pre[x])
        return res