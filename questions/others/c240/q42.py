class Solution(object):
    def largestPathValue(self, colors, edges):
        ...
        def helper(k):
            inDegree = list(inls)
            count =[0] * n  # How many color of k  at most there along the path from a start to i-th node
            q=[]
            for c in cand:
                count[c] = int(ord(colors[c])-ord('a') ==k)
                ans[0] = max(ans[0],count[c])
                q.append(c)
            while q:
                c = q[0]
                #print("la: " ,c)
                ...
                for p in g[c]:
                    #print(p)
                    count[p] = max(count[p],count[c]+ int(ord(colors[p])-ord('a') ==k))
                    ans[0] = max(ans[0],count[p])
                    inDegree[p] -=1
                    #print(inDegree,p)
                    if inDegree[p] ==0:
                        q.append(p)
            #print(inDegree,inls,g)
            for i in inDegree:
                if i >0 :
                    return -1
            else:
