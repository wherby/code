...
def offlineQuery(qrs,n,seq,getlcp):
    ...
    def update(x, length):
        if lcp_cache[x] < length:
            events.append(( x + lcp_cache[x],  -1))
            events.append(( x + length,  1))
            lcp_cache[x] = length
    ...
    for i in range(n - 1, -1, -1):
       ...
       # print(events,queries)
        while query_idx >= 0 and queries[query_idx][0] == i:
            current_query = queries[query_idx]
            query_ans = s_prefix[current_query[1] ]
            for event in events:
                pos = event[0]
                val = event[1]
                if pos <= current_query[1]:
                    query_ans += val * (current_query[1] - pos + 1)
            ans[current_query[2]] = query_ans
            query_idx -= 1
