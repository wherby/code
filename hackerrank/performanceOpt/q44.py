...
def dfs(g, v, p, target, path):
    ...
    if LCA(v,target) == v:
        ...
        path1 = dfsord[outdfs[v]:outdfs[target]+1]
    else:
        p = LCA(v,target)
        if indfs[v] < indfs[target]:
            path1= list(dfsord[outdfs[v]:outdfs[p]+1]) #list(reversed(dfsord[indfs[p]:indfs[v]+1]))
            path2 = list(dfsord[indfs[p]-1:indfs[target]+1])#list(reversed(dfsord[outdfs[target]:outdfs[p]]))
            path1.extend(path2)
        else:
