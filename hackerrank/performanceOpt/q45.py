...
def dfs(g, v, p, target, path):
    ...
    if LCA(v,target) == v:
        ...
        if indfs[v] < indfs[target]:
            path1= [[outdfs[v],outdfs[p]]] #list(reversed(dfsord[indfs[p]:indfs[v]+1]))
            path2 = [[indfs[p],indfs[target]+1]]#list(reversed(dfsord[outdfs[target]:outdfs[p]]))
            path1.extend(path2)
        else:
