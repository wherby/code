...
def getMapValue(chessListA,chessListB):
	updateChessDirty(chessListA,chessListB) 
	a1= getValueCHESS(chessListB)
	isAttacked = isQueenAttacked(chessListA,chessListB,1)
	if isAttacked:
		return 1000 -a1
	else:
		return 100 -a1
...
def moveOneChess(chessListA,chessListB,AorB):
	...
	mapTemp=[]
	if AorB == 1:
		for la,lb in mapListList:
			#print la,lb
			valueTemp = getMapValue(la,lb)
...
for i in range(q):
	...
	index = index +1 + n1
	solveChessDirty2(CHESSMAPLIST,c)
