...
nodes = []
for i in range(n +1):
    nodes.append([0,SortedCollection(key=operator.itemgetter(1))])
dic1={}
nodeOrd =[]
...
def addToNode(x,w):
    global nodes,dic1,sortedList
    sortedList.remove([x,nodes[x][0]])
    nodes[x][1].insert([w,[w]])
    nodes[x][0] = nodes[x][1][-1][0]
    sortedList.insert([x,nodes[x][0]])
    


def addToMaxNode(x,w):
    global nodes,sortedList
    sortedList.remove([x,nodes[x][0]])
    if len(nodes[x][1]) ==0:
        addToNode(x,w)
    else:
        node = nodes[x][1][-1]
        nodes[x][1].remove(node)
        node[1].append(w)
        node[0] = node[0] +w 
        nodes[x][1].insert(node)
    nodes[x][0] = nodes[x][1][-1][0]
    sortedList.insert([x,nodes[x][0]])
