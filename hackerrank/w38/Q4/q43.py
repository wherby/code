...
def removeNodeOrd(node):
    global nodeOrd
    i = bisect.bisect_left(KeyifyList(nodeOrd,lambda x:x[1]),node[1])
    if len(nodeOrd) > i and  nodeOrd[i][0] == node[0]:
...
def findFarest(x1):
    global nodes,n, dic1 ,nodeOrd,ring
    if x1==1:
        farNode =n
    else:
        farNode = x1 -1
    ...
    if len(nodeOrd) > 0:
        maxOrdNode = nodeOrd[-1]
        diffs = maxOrdNode[1] - ring[-1] + getBase(x1,maxOrdNode[0])
        i = bisect.bisect_left(KeyifyList(nodeOrd,lambda x:x[1]),diffs)
        for key,value in nodeOrd[i:]:
